...
SENSITIVE_VALUE_PATTERNS = [
    r"^sk-[a-zA-Z0-9]{20,}$",              # OpenAI API keys (classic)
    r"^sk-proj-[a-zA-Z0-9_-]{20,}$",       # OpenAI API keys (new project format)
...
# Pattern to detect credentials embedded in URL query strings
...
def sanitize_args(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in args.items():
        if _is_sensitive_key(key):
            ...
            # Scrub credentials from URL query strings
            sanitized[key] = _scrub_url_credentials(value)
...
def _has_url_credentials(value: str) -> bool:
    """Return True if the string looks like a URL containing credential query params."""
    return bool(_URL_CREDENTIAL_PATTERN.search(value))


def _scrub_url_credentials(value: str) -> str:
    """Replace credential values in URL query strings with [REDACTED]."""
    return _URL_CREDENTIAL_PATTERN.sub(r"\1\2=[REDACTED]", value)
...
class Sanitizer:
    """Backward-compatible sanitizer wrapper used by adapter code."""

    def sanitize_dict(self, value: Dict[str, Any]) -> Dict[str, Any]:
