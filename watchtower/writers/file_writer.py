...
# File permissions for security (Unix only)
DIR_PERMISSIONS = 0o700  # Owner: rwx, Group: ---, Others: ---
FILE_PERMISSIONS = 0o600  # Owner: rw-, Group: ---, Others: ---
...
class FileWriter(TraceWriter):
    ...
    def __init__(
        ...
        self._file_init_lock = threading.Lock()
    ...
    def write(self, event: Dict[str, Any]) -> None:
        ...
        with self._buffer_lock:
            ...
            flush_run_id = event.get("run_id", "unknown")

        # Release lock before flush — flush acquires the lock internally for buffer trim
        if should_flush:
            self._flush_buffer(flush_run_id)
    ...
    def _flush_buffer(self, run_id: str) -> None:
        ...
        for retry_attempt in range(max_retries):
            try:
                # Create/open file with correct permissions
                if not self._is_windows:
                    # On Unix: Use os.O_CREAT | os.O_WRONLY with mode
                    open_flags = os.O_CREAT | os.O_WRONLY | os.O_APPEND
                    fd = os.open(trace_file, open_flags, FILE_PERMISSIONS)
                    f = open(fd, "a", encoding="utf-8", closefd=True)
                else:
                    # On Windows: Use default permissions (handled by open())
                ...
                try:
                    ...
                    if lock_acquired:
                        try:
                            ...
                            logger.warning("Failed to release file lock for %s: %s", trace_file, e)
                    # Always close file
                    try:
                        f.close()
                    except OSError as e:
                        logger.warning("Failed to close trace file %s: %s", trace_file, e)

                # Success: atomically trim the events we just wrote from the front of the buffer
                with self._buffer_lock:
                    del self._buffer[: len(events_to_write)]
