...
TRACE_FILE_PATTERN = re.compile(r"^(\d{4}-\d{2}-\d{2})_([a-zA-Z0-9_-]+)\.jsonl$")

# Dead letter pattern: dead_letter_YYYY-MM-DD_HH-MM-SS.jsonl
DEAD_LETTER_PATTERN = re.compile(
    r"^dead_letter_(\d{4}-\d{2}-\d{2})_(\d{2}-\d{2}-\d{2})\.jsonl$"
)
...
def cleanup_dead_letter_files(
    trace_dir: str = "~/.watchtower/traces",
    retention_days: int = 7,
    dry_run: bool = False,
) -> Tuple[int, int]:
    """Delete dead-letter files older than retention period.

    Args:
        trace_dir: Base trace directory containing dead_letter/
        retention_days: Number of days to retain dead-letter files
        dry_run: If True, don't actually delete files

    Returns:
        Tuple of (deleted_count, bytes_freed)
    """
    dead_letter_dir = get_trace_dir(trace_dir) / "dead_letter"
    ...
    try:
        entries = list(dead_letter_dir.iterdir())
    except OSError:
        return (0, 0)
    ...
    for entry in entries:
        ...
        match = DEAD_LETTER_PATTERN.match(entry.name)
        if not match:
            continue
