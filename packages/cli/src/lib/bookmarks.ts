...
/**
 * Get the home directory for Watchtower data.
 * WATCHTOWER_HOME overrides os.homedir() (useful for tests and custom installs).
 */
function getHomeDir(): string {
	return process.env['WATCHTOWER_HOME'] ?? os.homedir();
}
...
function getBookmarksPath(): string {
	return path.join(getHomeDir(), BOOKMARKS_DIR, BOOKMARKS_FILE);
}
...
function ensureBookmarksDir(): void {
	const dir = path.join(getHomeDir(), BOOKMARKS_DIR);
