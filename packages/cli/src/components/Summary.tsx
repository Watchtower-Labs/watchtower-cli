...
export function Summary({
	...
	const throttledEvents = liveStats?.throttledEvents ?? 0;
	const showThrottle = throttledEvents > 0 || liveStats?.isThrottling;
		...
		>
			...
			{showThrottle && (
				<StatItem
					icon="⏱"
					label="Throttled"
					value={throttledEvents}
					color={liveStats?.isThrottling ? 'yellow' : 'gray'}
				/>
			)}
			{errors > 0 ? (
				<StatItem
