...
export default function TracesPage() {
  ...
  const {data, isLoading, error} = useSWR<{traces: Trace[]}>('/api/traces', fetcher, {
    refreshInterval: POLL_INTERVAL_MS,
  })
  const allTraces: Trace[] = data?.traces ?? []
  const agents = Array.from(new Set(allTraces.map(t => t.agent_name)))
  ...
  const filteredTraces = useMemo(() => {
    ...
    return traces
  }, [agentFilter, searchQuery, allTraces])
          ...
          <motion.div variants={fadeIn} className="mb-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  type="text"
                  aria-label="Search traces by run ID, filename, or agent name"
            ...
            {isLoading ? (
              ...
              <div className="p-8 rounded-2xl border border-red-500/20 bg-red-500/5 text-center">
                <p className="text-red-400 text-sm">
                  Failed to load traces. Check that your{' '}
                  <code className="font-mono">WATCHTOWER_TRACE_DIR</code> is configured correctly.
                </p>
              </div>
            ) : (
              <TraceList
