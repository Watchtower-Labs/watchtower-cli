...
export default function TraceDetailPage() {
  ...
  const {data, isLoading, error} = useSWR<{trace: Trace}>(
    params.id ? `/api/traces/${params.id}` : null,
    fetcher
  )
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          <p className="text-red-400 mb-4">Failed to load trace data.</p>
          <p className="text-gray-500 text-sm">Check your server configuration and try again.</p>
        </div>
      </div>
    )
  }
