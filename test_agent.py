#!/usr/bin/env python3
"""
Test agent for validating Watchtower SDK and CLI integration.

This script creates a simple ADK agent with tracing enabled to test:
1. Trace file generation
2. Event capture (run, llm, tool events)
3. CLI integration (show, tail, list commands)
...
try:
    ...
    print("Install with: pip install -e . from the feature/phase1-sdk-core branch")
    sys.exit(1)
...
def search_knowledge_base(query: str, max_results: int = 5) -> list[dict]:
    ...
    # Simulated results
    results = [
        {"title": "Result 1", "content": f"Information about {query}"},
        {"title": "Result 2", "content": f"More details on {query}"},
        {"title": "Result 3", "content": f"Additional context for {query}"},
    ][:max_results]

    print(f"  [Tool] Found {len(results)} results for: {query}")
    return results
...
async def run_test_agent():
    ...
    print("=" * 70)
    print("Watchtower SDK + CLI Integration Test")
    print("=" * 70)
    print()
    ...
    agent = Agent(
        ...
        tools=[simple_calculator, get_current_time, search_knowledge_base],
    )
    ...
    test_queries = [
        ...
        "Search the knowledge base for information about ADK agents",
        "Calculate 100 divided by 4, then tell me what time it is",
    ]
